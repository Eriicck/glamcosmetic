...
// ── Modal de producto ──────────────────────────────────────
// ✅ Centrado en mobile (no bottom-sheet)
// ✅ Máximo 90dvh
...
function ProductModal({ product, onClose, onAddToCart }) {
  ...
  useEffect(() => {
    if (product) { setQty(1); setActiveImg(0); }
  }, [product]);
  ...
  return (
    // ✅ items-center en todas las pantallas → modal centrado verticalmente
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-6">
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm" onClick={onClose}/>

      {/* ✅ max-h-[90dvh] y rounded-2xl en todas las pantallas */}
      <div className="relative w-full md:max-w-3xl bg-white rounded-2xl overflow-hidden shadow-2xl"
        style={{ maxHeight: '90dvh', overflowY: 'auto' }}>

        {/* Botón cerrar */}
        <button onClick={onClose}
          className="absolute top-3 right-3 z-10 w-9 h-9 rounded-full bg-white/90 backdrop-blur-sm flex items-center justify-center hover:bg-gray-100 transition-colors shadow-sm">
          ...
          <div className="p-6 md:p-8 flex flex-col justify-between">
            <div>
              ...
              <h2 className="text-xl md:text-2xl font-bold text-gray-900 mt-2 leading-snug"
                style={{fontFamily:"'Playfair Display',serif"}}>{product.name}</h2>
              ...
              <p className="text-sm text-gray-500 mt-4 leading-relaxed">
                {product.description||'Producto de belleza premium. Calidad garantizada para que luzcas increíble.'}
              </p>
              {product.hasTon && product.tonValue && (
                <div className="mt-3 inline-flex items-center gap-2 bg-pink-50 text-[#D2006E] text-xs font-semibold px-3 py-1 rounded-full">
                  💄 Tono: {product.tonValue}
                </div>
              )}
            </div>
            ...
            <div className="mt-6 space-y-3">
              ...
              <button disabled={product.stock===false}
                ...
                className={`w-full text-white py-3.5 text-sm font-bold tracking-widest rounded-xl transition-all duration-300 shadow-md flex items-center justify-center gap-2
                  ${product.stock!==false?'bg-[#D2006E] hover:bg-[#E5108A]':'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
    ...
    <div className="group flex flex-col cursor-pointer" onClick={() => onOpenModal(product)}>
      <div className="relative overflow-hidden rounded-2xl bg-[#FDF4F8] mb-3" style={{aspectRatio:'3/4'}}>
        {product.image
          ...
          : <div className="w-full h-full flex items-center justify-center text-gray-300"><ShoppingCart size={32} strokeWidth={1}/></div>}
        ...
        <div className="absolute bottom-3 left-3 right-3 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0">
          <button disabled={product.stock===false}
            ...
            className={`w-full py-2.5 text-xs font-bold tracking-widest uppercase rounded-xl transition-colors flex items-center justify-center gap-2
              ${product.stock!==false?'bg-[#D2006E] text-white hover:bg-[#E5108A]':'bg-gray-300 text-gray-500 cursor-not-allowed'}`}>
      ...
      <div className="px-0.5">
        ...
        <div className="flex items-center gap-2">
          {hasOffer
            ? <><span className="text-base font-bold text-[#D2006E]">${Number(product.offerPrice).toFixed(2)}</span><span className="text-sm text-gray-400 line-through">${Number(product.price).toFixed(2)}</span></>
...
export default function Products({ cartCount = 0, onOpenCart, onAddToCart }) {
  ...
  const handleAddToCart = useCallback((product,qty=1) => { onAddToCart?.(product,qty); }, [onAddToCart]);
    ...
    <div className="min-h-screen bg-white text-gray-900 overflow-x-hidden" style={{fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <header className="sticky top-0 z-40 w-full bg-white/95 backdrop-blur-sm border-b border-pink-100">
        <div className="flex items-center justify-between px-4 py-3 md:px-8 max-w-7xl mx-auto">
          ...
          <button onClick={()=>navigate('/')} className="font-bold text-xl tracking-widest" style={{fontFamily:"'Playfair Display',serif"}}>GLAM</button>
          <div className="flex items-center justify-end gap-4 flex-1">
            <button onClick={onOpenCart} className="relative hover:opacity-70 transition-opacity">
              <ShoppingCart size={20} strokeWidth={1.5}/>
              {cartCount>0 && <span className="absolute -bottom-2 -right-2 flex h-5 w-5 items-center justify-center rounded-full bg-[#D2006E] text-[10px] font-bold text-white">{cartCount}</span>}
            </button>
          </div>
        </div>
      </header>
      ...
      <main className="w-full max-w-7xl mx-auto px-4 md:px-8 py-8">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-10 md:grid-cols-3 lg:grid-cols-4 md:gap-x-6 md:gap-y-12">
            {currentProducts.map(p=><ProductCard key={p.id} product={p} onAddToCart={handleAddToCart} onOpenModal={setSelectedProduct}/>)}
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="relative ml-auto w-72 bg-white h-full overflow-y-auto shadow-2xl flex flex-col">
                  ...
                  <div className="space-y-3 pt-1">
                    {ALL_CATS.map(c=>(
                      <label key={c} className="flex items-center gap-3 cursor-pointer group">
                        <input type="checkbox" checked={selectedCats.includes(c)} onChange={()=>toggleCat(c)} className="w-4 h-4 rounded accent-[#D2006E]"/>
                  ...
                  <div className="space-y-3 pt-1 max-h-48 overflow-y-auto pr-1">
                    {ALL_BRANDS.map(b=>(
                      <label key={b} className="flex items-center gap-3 cursor-pointer group">
                        <input type="checkbox" checked={selectedBrands.includes(b)} onChange={()=>toggleBrand(b)} className="w-4 h-4 rounded accent-[#D2006E]"/>
            ...
            <div className="border-t border-pink-100 p-4 space-y-2">
              <button onClick={()=>setFilterOpen(false)} className="w-full bg-[#D2006E] text-white py-3 text-sm font-bold tracking-widest hover:bg-[#E5108A] transition-colors rounded-xl">APLICAR</button>
              <button onClick={()=>{clearFilters();setFilterOpen(false);}} className="w-full text-sm text-gray-500 py-2 hover:text-[#D2006E] transition-colors">Eliminar todo</button>
            </div>
          </div>
        </div>
      )}
