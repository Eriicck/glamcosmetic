  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 overflow-x-hidden" style={{ fontFamily: "'DM Sans', sans-serif" }}>
            ...
            <nav className="space-y-6">
              {NAV_LINKS.map(l => (
                <button key={l.label} onClick={() => {
                  setMenuOpen(false);
                  l.path.startsWith('#') ? document.querySelector(l.path)?.scrollIntoView({ behavior: 'smooth' }) : goTo(l.path);
      ...
      <section className="relative h-[100dvh] min-h-[600px] flex items-center justify-center overflow-hidden bg-[#0C0210]">
        ...
        {/* Slide indicators (solo en modo slides con >1 imagen) */}
            ...
            <div className="grid grid-cols-3 gap-3 md:gap-5">
              {editorial.map((item, i) => (
                <button key={item.id} onClick={() => goTo(item.link || '/products')}
                  className={`group relative overflow-hidden rounded-2xl bg-gray-100 transition-all duration-700 ${i === activeEditorial ? 'col-span-2' : 'col-span-1'}`}
      ...
      <section className="relative py-28 text-center px-5 overflow-hidden bg-[#FDF4F8]">
        {cta?.src && <img src={cta.src} alt="" className="absolute inset-0 w-full h-full object-cover" />}
        {cta?.src && <div className="absolute inset-0" style={{ background: `rgba(12,2,16,${(cta.overlayOpacity||40)/100})` }} />}
        <div className="relative z-10">
          <p className="text-[#D2006E] text-[10px] tracking-[0.5em] uppercase mb-4 font-semibold">{cta?.eyebrow || 'Explorar'}</p>
          <h2 className="glam-title text-5xl md:text-7xl font-bold mb-2 text-[#0D0D12]">{cta?.title || 'Tu look,'}</h2>
          <h2 className="glam-title text-5xl md:text-7xl italic font-normal text-[#D2006E] mb-10">{cta?.titleItalic || 'perfecto.'}</h2>
          <button onClick={() => goTo('/products')}
